...
func (w *WalletController) Add(userId int64, amount float64, currencyId int64, currencyType string, nickname string, primary bool) error {
	...
	if err == nil {
		...
		if err == nil {
			if notDeleted {
				...
				wallet := models.Wallet{WalletMasterId: walletMaster.Id, Amount: amount, AmountLocked: 0, Nickname: nickname, Primary: primary}
				err := wallet.Insert()
				if err == nil {
